...
const releaseCatalog: KnowledgeNode[] = [
  { id: 'release:v2.2.3', type: 'release', title: 'v2.2.3 hybrid classifier and transcript reliability', summary: 'Добавляет semantic fusion, auxiliary classifiers, ML shadow review и continuation для длинных облачных AI-ответов без изменения Live Shield audio lifecycle.', tags: ['release', 'ml', 'deepfake', 'robocall', 'safety'], version: '2.2.3', status: 'active' },
...
export function buildKnowledgeGraph(storage: ModelStorageInfo | null = null, runtime: KnowledgeRuntime = {}): KnowledgeGraph {
  ...
  modelNodes.push(
    ...
    { id: 'model:silero-vad', type: 'model', title: 'Silero VAD', summary: 'Sherpa-compatible bundled ONNX для offline оценки наличия речи и качества аудио. Не подключён к Live Shield до Xiaomi benchmark.', tags: ['vad', 'audio', 'onnx'], status: 'available', bytes: 643_854 },
    { id: 'model:aasist', type: 'model', title: 'AASIST', summary: 'Bundled ONNX synthetic-voice evidence через ONNX Runtime. ASVspoof2019 LA checkpoint, не calibrated для RU/KZ телефонных звонков и не меняет live risk.', tags: ['deepfake', 'audio', 'onnx', 'asvspoof'], status: 'available', bytes: 1_615_195 },
  ...
  const edges: KnowledgeEdge[] = ([
    ...
    ['app:voiceshield', 'release:v2.2.3', 'current release'], ['release:v2.2.3', 'release:v2.2.2', 'succeeds'], ['release:v2.2.2', 'release:v2.2.1', 'succeeds'], ['release:v2.2.1', 'release:v2.2.0', 'succeeds'],
    ...
    ['feature:quality-lab', 'dataset:kazakh-asr', 'evaluates against'], ['feature:quality-lab', 'dataset:common-voice', 'evaluates against'], ['feature:quality-lab', 'model:silero-vad', 'offline evidence'], ['feature:quality-lab', 'model:aasist', 'offline evidence'], ['feature:quality-lab', 'model:lcnn-anti-spoof', 'candidate evaluation'], ['feature:ml-shadow', 'dataset:fraud-transfer', 'trained with'], ['model:lcnn-anti-spoof', 'dataset:asvspoof', 'trained/evaluated on'], ['model:aasist', 'dataset:asvspoof', 'trained/evaluated on'],
...
// The assistant needs the complete feature/release catalog to answer questions
// about version differences. Callers can still pass a lower bound for a small
// prompt, while the product default no longer silently drops older releases.
export function buildAssistantKnowledgeContext(graph: KnowledgeGraph, maxNodes = 64): string {
