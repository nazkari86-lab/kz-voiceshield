...
function AppContent() {
  ...
  const content = (
    <>
      {tab === 'live' && <LiveView analysis={w.analysis} rawAnalysis={w.rawAnalysis} modelCorrection={w.modelCorrection} transcript={w.transcript} enhancement={w.transcriptEnhancement} source={w.source} modelReady={w.modelReady} isListening={w.isListening} audioLevel={w.audioLevel} error={w.captureError} notice={w.captureNotice} callStatus={w.callStatus} storageError={w.storageError} trustedContactName={w.trustedContact?.name} callbackWarning={w.callbackInfo?.warning} liveAi={liveAi} onChangeTranscript={w.setTranscript} onToggleListening={() => { void (w.isListening ? w.stopListening() : w.startListening()) }} onUseMicrophoneFallback={() => { void w.switchToMicrophoneFallback() }} onEndCall={w.endActiveCall} onSave={w.saveCurrentCase} onExportReport={w.exportReport} onCallTrusted={() => { void w.callTrustedContact() }} onOpenEmergency={() => selectTab('emergency')} onOpenSimulator={() => selectTab('simulator')} onOpenAi={() => selectTab('llm')} />}
