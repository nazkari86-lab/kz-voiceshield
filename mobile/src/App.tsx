...
const toolTabs: Array<[Tab, string, string]> = [
  ...
  ['account', 'Account & family', 'Device account and family protection'],
...
const tabMeta: Record<Tab, { label: string; group: string }> = {
  ...
  account: { label: 'Account & family', group: 'Protect' },
}
...
function AppContent() {
  ...
  const content = (
    <>
      ...
      {tab === 'dataset' && <DatasetView cases={w.cases} quality={w.quality} caseCount={w.cases.length} donationReadiness={w.donationReadiness} datasetStageTotals={w.datasetStageTotals} donationConsent={w.donationConsent} onSetDonation={(accepted) => { void w.setDonation(accepted) }} onDonate={() => { void w.donateDataset() }} onExportJsonl={w.exportJsonlCases} onExportCsv={w.exportCsvCases} onExportSplit={w.exportSplitCases} onClear={w.clearCases} />}
