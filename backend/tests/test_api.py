...
def api(tmp_path: Path):
    ...
    app = create_app(
        settings=settings, transcriber=FakeTranscriber(), model_service=FakeModel()
    )
...
def test_health_is_public_and_auth_is_required(api):
    ...
    assert (
        client.post(
            "/analyze-transcript", json={"transcript": "Назовите код"}
        ).status_code
        == 401
    )
...
def test_case_storage_is_encrypted_and_role_guarded(api):
    ...
    response = client.put(
        "/cases/case-1", headers=auth("analyst-token"), json=case_payload()
    )
    assert response.status_code == 200
    ...
    with sqlite3.connect(database_path) as connection:
        encrypted = connection.execute(
            "SELECT payload FROM cases WHERE id = 'case-1'"
        ).fetchone()[0]
...
def test_reviewer_updates_workflow_and_reads_audit(api):
    ...
    assert (
        client.put(
            "/cases/case-1", headers=auth("analyst-token"), json=analyst_overwrite
        ).status_code
        == 200
    )
    preserved = client.get(
        "/cases?status=escalated", headers=auth("reviewer-token")
    ).json()["items"][0]
...
def test_workflow_patch_rejects_stale_reviewer_version(api):
    ...
    client.put(
        "/cases/case-1",
        headers=auth("analyst-token"),
        json={**case_payload(), "updatedAt": "2026-07-11T00:00:00Z"},
    )
...
def test_audio_job_completes_without_retaining_audio(api):
    ...
    job = client.get(
        f"/audio-jobs/{accepted.json()['jobId']}", headers=auth("analyst-token")
    )
    ...
    with sqlite3.connect(database_path) as connection:
        retained_audio = connection.execute("SELECT audio FROM audio_jobs").fetchone()[
            0
        ]
...
def test_rejects_large_or_unsupported_audio(api):
    ...
    unsupported = client.post(
        "/transcribe-audio",
        headers=auth("analyst-token"),
        files={"audio": ("call.txt", b"voice", "text/plain")},
    )
    assert unsupported.status_code == 415
    too_large = client.post(
        "/transcribe-audio",
        headers=auth("analyst-token"),
        files={"audio": ("call.wav", b"x" * 65, "audio/wav")},
    )
    assert too_large.status_code == 413


def test_analyze_transcript_returns_200_when_ml_unavailable(tmp_path: Path):
    """v2.1.0: /analyze-transcript must never 503 when model is missing.
    It returns 200 with ml: null and mlAvailable: false so rule-based scoring
    remains the authoritative signal even without the ML model deployed."""

    class NoModel:
        available = False

        def assess(self, transcript: str):  # noqa: ARG002
            from backend.app.ml_service import ModelUnavailable
    ...
    app = create_app(settings=settings, model_service=NoModel())
    with TestClient(app) as client:
        response = client.post(
            "/analyze-transcript",
            headers=auth("analyst-token"),
            json={"transcript": "Назовите код"},
        )
    assert response.status_code == 200
    body = response.json()
